...
* MODIFIED: Wed 08 Aug 2018 01:36:55 PM EDT
...
void filter_color ( Mat &orig_img, Mat &out_img, vector<int> &filters, bool debug=false )
{
  ...
  Mat color_mask_1, color_mask_2, color_mask;
...
/*  // Filter values 
  ...
  for (unsigned int i=0; i<locations.size(); i++){
    if (debug) circle(img_display, locations[i], 50, 200);
    circle(full_mask, locations[i], 50, 255, -1); // radius 50
  }
